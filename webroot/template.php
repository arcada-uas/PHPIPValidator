...
<div>
    ...
    <?php if (!$template['isRegistered']) : ?>
        ...
        <h2>Register for <?= $template['courses'][$template['course']] ?> :: <?= date("Y-m-d") ?></h2>
        <form method="POST">
    ...
    <?php else : ?>
        ...
        <h2>You registered for todays <?= $template['courses'][$template['isRegistered']['course']] ?> lecture at <?= $template['isRegistered']['date'] ?></h2>
    <?php endif; ?>
</div>
</section>
